...
const HomePage = () => {
  ...
  const fetchUpdatedTeamData = async () => {
    try {
      const response = await fetch(`${API_URL}/api/team/${teamData.id}`);
  ...
  useEffect(() => {
    ...
    socket.on("auctionEnded", async () => {
      if (biddingPlayer && highestBidder && currentBid > 0) {
        try {
          await fetch(`${API_URL}/api/place-bid`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
  ...
  const placeBid = (playerId) => {
    if (timer > 0) {
      ...
      // First bid can match base price; subsequent bids must exceed current bid
      const bidAboveMinimum = highestBidder === null ? bidAmount >= currentBid : bidAmount > currentBid;
      if (bidAboveMinimum && bidAmount <= budget) {
      ...
      {showPlayerModal && selectedPlayer && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <img src={`${API_URL}/images/${selectedPlayer.image_url}`} alt={selectedPlayer.name} className="player-img" />
